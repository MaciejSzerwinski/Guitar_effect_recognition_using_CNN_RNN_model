...
#Klasa klasyfikująca efekt gitarowy na podstawie ich cech spektralnych
...
#Klasa inicjalizująca model sieci neuronowej zprojektowanej do estymacji
# ustawień (parametrów) efektów dźwiękowych na podstawie spektrogramu i
# dodatkowych informacji o klasie efektu. Model ten wykorzystuje zarówno
# warstwy konwolucyjne, jak i gęste (fully connected), a także warstwę
# osadzeń (embedding layer) do kondycjonowania przewidywań na podstawie
# klasy efektu.
class SettingsNetCond(nn.Module):
    ...
    def __init__(self, n_settings, mel_shape, num_embeddings, embedding_dim=50):
        ...
        self.num_embeddings = num_embeddings #liczba możliwych klas efektów (rozmiar słownika osadzeń)
        self.embedding_dim = embedding_dim #Wymiar wektora osadzeń (rozmiar wektora dla każdej klasy efektu).
    ...
    def forward(self, t, c):
        ...
        print(f"Shape of c before reshape: {c.size()}")
        # Reshape c to match the batch size of t
        c = c.reshape(t.size(0), 1, self.mel_shape[0], self.mel_shape[1])

        # If c has different spatial dimensions, resize it using interpolation
        c = F.interpolate(c, size=(t.size(2), t.size(3)), mode='bilinear', align_corners=False)

        # Now c should have the same spatial dimensions as t
        c = c.expand(t.size(0), t.size(1), t.size(2), t.size(3))
