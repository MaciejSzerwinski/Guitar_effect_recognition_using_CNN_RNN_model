...
def plot_confusion_matrix(cm, classes, normalize=False, title='Confusion matrix', title_fontsize=20, text_fontsize=16,
                          cmap=plt.cm.Blues):
    if normalize:
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        plt.text(j, i, format(cm[i, j], fmt), size=text_fontsize, horizontalalignment="center",
