...
def train_fx_net(model, optimizer, train_loader, train_sampler, epoch, loss_function=nn.CrossEntropyLoss(), device='cpu'):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        # Usuń wymiar kanałów i przekształć dane do formatu [batch_size, sequence_length, input_size]
        mels = mels.squeeze(2).to(device)  # mels ma teraz kształt [batch_size, height, width]
        #print("Shape of mels before passing to model:", mels.shape)  # Sprawdzenie kształtu danych
        labels = labels.to(device)

        preds = model(mels)  # Przekazanie sekwencji do modelu RNN
        ...
        if batch_idx > 0 and batch_idx % 50 == 0:
            print('Train Epoch: {}\t[{}/{} ({:.0f}%)]\tTotal Loss: {:.4f}\tAvg Loss: {:.4f}'.format(
                ...
                train_set_size,
                100. * batch_idx / len(train_loader),  # % ukończenia
                total_loss,
                total_loss / (batch_idx * len(labels))))

    print(
        '====> Epoch: {}\tTotal Loss: {:.4f}\t Avg Loss: {:.4f}\tCorrect: {:.0f}/{}\tPercentage Correct: {:.2f}'.format(
            epoch,
            total_loss,
...
def val_fx_net(model, val_loader, val_sampler, loss_function=nn.CrossEntropyLoss(), device='cpu'):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(val_loader):
            ...
            # Usuń wymiar kanałów, aby dane miały kształt [batch_size, sequence_length, input_size]
            mels = mels.squeeze(2).to(device)
            labels = labels.to(device)
...
def train_settings_net(model, optimizer, train_loader, train_sampler, epoch, loss_function=nn.L1Loss(), device='cpu'):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        mels = mels.squeeze(2).to(device)  # mels ma teraz kształt [batch_size, height, width]
        #print(f"Shape of mels before passing to model: {mels.shape}")
    ...
    print(
        '====> Epoch: {}\tTotal Loss: {:.4f}\t Avg Loss: {:.4f}\tCorrect: {:.0f}/{:.0f}\tPercentage Correct: {:.2f}'.format(
            epoch,
            total_loss,
            total_loss / train_set_size,
            total_correct,
...
# ===============================================================
def train_settings_cond_net(model, optimizer, train_loader, train_sampler, epoch, loss_function=nn.L1Loss(),
                            device='cpu'):
    model.train()
    train_set_size = len(train_sampler)
    total_loss = 0
...
def val_multi_net(model, val_loader, val_sampler, loss_function_fx=nn.CrossEntropyLoss(), loss_function_set=nn.L1Loss(),
                  device='cpu'):
    model.eval()
    val_set_size = len(val_sampler)
    total_loss_fx = 0
...
def test_multi_net(model, test_loader, test_sampler, loss_function_fx=nn.CrossEntropyLoss(),
                   loss_function_set=nn.L1Loss(), device='cpu'):
    model.eval()
    test_set_size = len(test_sampler)
    total_loss_fx = 0
    total_loss_set = 0
...
def train_cond_nets(model_fx, model_set,
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        preds_fx = model_fx(mels)  # Przekazanie sekwencji do modelu RNN
        loss_fx = loss_function_fx(preds_fx, labels)
        ...
        cond_set = preds_fx.argmax(dim=1)  # Obliczenie etykiet do kondycjonowania SettingsNet
        preds_set = model_set(mels, cond_set)  # Przekazanie sekwencji do modelu RNN
        loss_set = loss_function_set(preds_set, settings)  # Obliczenie straty
